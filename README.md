...
Obsidian公式コミュニティプラグインディレクトリに公開済みです。

1. Obsidianの設定 →「コミュニティプラグイン」→「閲覧」
2. 検索欄で「Oden」と検索
3. 該当プラグインを選び「インストール」→ インストール完了後「有効化」

以後のアップデートは、設定→「コミュニティプラグイン」の更新チェック（🔄）から通常のプラグイン同様に行えます。

### 開発版・手動インストール（コントリビューター向け）

このリポジトリのソースから直接試したい場合は以下の手順です。
...
- [x] Obsidian公式コミュニティプラグインディレクトリへの公開
